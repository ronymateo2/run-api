...
async function signJwt(payload, secret, expiresIn = "30d") {
...
auth.post("/google", async (c) => {
    ...
    const existing = await c.env.DB.prepare(`SELECT u.* FROM users u
     JOIN user_auth_providers p ON p.user_id = u.id
     WHERE p.provider = 'google' AND p.provider_sub = ?`).bind(googleSub).first();
    let userId;
    if (existing) {
        userId = existing.id;
        // Update name/avatar if changed
        await c.env.DB.prepare(`UPDATE users SET name = ?, avatar_url = ?, updated_at = ? WHERE id = ?`).bind(name, picture, now, userId).run();
    }
    else {
        // Create user + auth provider link
        userId = crypto.randomUUID();
        await c.env.DB.batch([
            c.env.DB.prepare(`INSERT INTO users (id, email, name, avatar_url, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`).bind(userId, email, name, picture, now, now),
            c.env.DB.prepare(`INSERT INTO user_auth_providers (id, user_id, provider, provider_sub, created_at)
         VALUES (?, ?, 'google', ?, ?)`).bind(crypto.randomUUID(), userId, googleSub, now),
        ]);
    }
