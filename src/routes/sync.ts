...
const TABLE_SPECS: TableSpec[] = [
  ...
  // the conflict WHERE guards edits to exercises under the user's own phases.
  {
    ...
    build: (db, row, userId, now) =>
      db.prepare(
        `INSERT INTO exercises (id, phase_id, name, detail, sets, reps, duration_s, exercise_type, sort_order, video_url, warmup_sets, archived_at, created_at, updated_at, client_updated_at)
         SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
         WHERE EXISTS (
        ...
        row.warmup_sets ?? 0, row.archived_at ?? null, now, now, row.client_updated_at ?? null,
