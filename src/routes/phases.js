...
phases.get("/", async (c) => {
    ...
    const injuries = await c.env.DB.prepare(`SELECT * FROM injuries WHERE user_id = ?`).bind(userId).all();
    const phaseRows = await c.env.DB.prepare(`SELECT p.*, pc.id as crit_id, pc.description as crit_desc, pc.done as crit_done
     FROM phases p
     LEFT JOIN phase_criteria pc ON pc.phase_id = p.id
     JOIN injuries i ON i.id = p.injury_id
     WHERE i.user_id = ?
     ORDER BY p.injury_id, p.phase_num, pc.id`).bind(userId).all();
    ...
    const exercises = await c.env.DB.prepare(`SELECT e.phase_id, COUNT(DISTINCT el.id) as completed_sessions
     FROM exercises e
     LEFT JOIN exercise_logs el ON el.exercise_id = e.id
     JOIN phases p ON p.id = e.phase_id
     JOIN injuries i ON i.id = p.injury_id
     WHERE i.user_id = ?
     GROUP BY e.phase_id`).bind(userId).all();
